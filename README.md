...
The stage shows only a half-hidden brand tab bobbing at the bottom edge — everything else is quiet chrome. The tab opens display settings, which holds view modes, arrows, fullscreen, curtain recall, shortcuts, and the in-gallery information sheet (position, caption, EXIF, Content Credentials). `I` deep-links the current photograph into the standalone C2PA viewer (`c2pa.thecontrarian.in/?uri=…`) in a new tab, falling back to the in-gallery sheet for sources it cannot fetch (videos, non-http origins). Navigation arrows ship on for fine pointers and hide by default on touch — enabled there, they dock at the viewport's bottom corners. The viewer supports strip, vertical-scroll, and one-at-a-time modes, pointer and touch dragging, wheel input, keyboard navigation, and reduced-motion preferences — images fit the stage without cropping and are never upscaled past their natural size. Sizing is density-aware rather than purely CSS-driven: the horizontal strip fits height-first so neighbouring photographs abut edge-to-edge like a photostrip (its only ceiling is the source's own pixel count), vertical scroll fits width-first (portraits included — a tall frame simply runs long) with the effective device pixel ratio capped at 2, and one-at-a-time contains both axes under the same cap — so a displayed pixel never claims more source pixels than exist. Beneath the photographs sits a stable plain canvas, independent of the optional background treatment. The viewer applies no provider transforms and preserves original bytes; where a source serves only derivatives — iCloud web derivatives, Drive/MEGA previews for formats the browser cannot decode, or the existing HEIC fallback — the safe display size caps to the actual served pixels, with no new canvas re-encoding.
