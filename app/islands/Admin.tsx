    ...
    if (isEditing) {
      const common = {
        ...
        onKeyDown: (event: KeyboardEvent) => {
          ...
          if (event.key === 'Enter' && field === 'title') { event.preventDefault(); void saveEditing() }
  ...
  return (
    <main class="admin-page admin-page--selector">
      <header class="admin-header">
        <div>
          <h1>manorama.xyz</h1>
          <p class="admin-intro">Simply the wow-est way to share photos with anyone!</p>
        </div>
      ...
      <section class="gallery-list" aria-label="Published galleries">
        {galleries.length ? <div class="admin-gallery-list">{galleries.map((gallery) => <article class="admin-gallery-card" key={gallery.slug}>
          <div class="admin-gallery-card-body"><div class="admin-gallery-title-row">{editableText(gallery, 'title', 'admin-gallery-title')}<span class="admin-gallery-count" aria-label={`${gallery.imageCount} photographs`}>({gallery.imageCount})</span></div>{editableText(gallery, 'caption', 'admin-gallery-caption')}</div>
          ...
          <div class="gallery-card-actions"><a class="admin-gallery-url" href={galleryPath(gallery.slug)} target="_blank" rel="noreferrer">{galleryAddress(gallery.slug)}</a><button type="button" class="admin-icon-action" title="Copy gallery link" aria-label={`Copy ${gallery.title} link`} onClick={() => copyGalleryAddress(gallery)}><CopyIcon /></button>{gallery.sourceUrl ? <button type="button" class="admin-icon-action admin-icon-action--delete" title="Delete gallery" aria-label={`Delete ${gallery.title}`} onClick={() => removeGallery(gallery)} disabled={busy}><TrashIcon /></button> : null}</div>
